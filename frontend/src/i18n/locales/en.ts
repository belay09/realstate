export const en = {
  brand: {
    name: 'Habesha Real Estate Advisory',
    shortName: 'Habesha Homes',
    tagline: 'Free real estate advisory',
    headerNote: 'Not a developer',
  },
  nav: {
    home: 'Home',
    listings: 'Listings',
    apartments: 'Apartments',
    shops: 'Shops',
    calculator: 'Calculator',
    ayat: 'Ayat ↗',
    ayatBrowse: 'Ayat homes',
    temer: 'Temer homes',
    staff: 'Staff',
    compareHomes: 'Compare homes',
    browseHomes: 'Browse homes',
  },
  contact: {
    label: 'Contact',
    call: 'Call',
    whatsapp: 'WhatsApp',
    bannerHint: 'Call or WhatsApp for any listing enquiry.',
    regionLabel: 'Contact Habesha Real Estate Advisory',
  },
  footer: {
    contact: 'Contact',
    contactHint: 'Call or WhatsApp for any listing enquiry.',
    explore: 'Explore',
    developers: 'Developers',
    partners: 'Partner sites',
    tagline: 'Compare homes · Choose your developer',
    allListings: 'All listings',
    apartments: 'Apartments',
    shops: 'Shops',
    ayatHomes: 'Ayat homes',
    temerHomes: 'Temer homes',
    calculator: 'Price calculator',
    officialAyat: 'Official Ayat website ↗',
    officialTemer: 'Official Temer website ↗',
    description:
      'A comparison platform for Ethiopian real estate. We are not a developer. Listings come from partners such as {{ayatBrand}} and {{temerBrand}}. Each home shows who built it so you can choose with confidence.',
    descriptionSlim:
      'Free buyer advisory for {{ayatBrand}} and {{temerBrand}}. We are not a developer.',
    copyright: 'Compare homes from multiple developers',
  },
  theme: {
    light: 'Switch to light mode',
    dark: 'Switch to dark mode',
  },
  language: {
    switch: 'Language',
    english: 'English',
    amharic: 'Amharic',
  },
  pageTitles: {
    home: 'Home',
    listings: 'Browse homes',
    apartments: 'Apartment locations',
    shops: 'Shop locations',
    listing: 'Home details',
    calculator: 'Price calculator',
    calculatorPaused: 'Pricing guidance',
    developerKind: '{{brand}} — choose type',
    developerShops: '{{brand}} shop locations',
  },
  seo: {
    defaultDescription:
      'Free real estate advisory in Addis Ababa. Compare Ayat and Temer apartments and shops. No buyer fees, no commission — Habesha Real Estate Advisory.',
    homeDescription:
      'Habesha Real Estate Advisory helps you find the best home for your budget and lifestyle in Addis Ababa. Compare Ayat and Temer listings — 100% free, no commission from buyers.',
    apartmentsDescription:
      'Browse apartment locations from Ayat Share Company and Temer Properties in Addis Ababa. Free advisory to help you pick the right area and layout.',
    shopsDescription:
      'Ayat commercial shop locations in Addis Ababa with official floor rates. Free guidance from Habesha Real Estate Advisory.',
    developerKindDescription:
      'Choose Residential or Shops from {{brand}}. Free buyer advisory from Habesha Real Estate Advisory.',
    developerShopsDescription:
      'Browse {{brand}} shop locations in Addis Ababa. Free guidance from Habesha Real Estate Advisory.',
    calculatorDescription:
      'Estimate indicative Ayat apartment and shop prices in Addis Ababa using official strategy bands. Free tool — confirm quotes with Ayat before purchase.',
    calculatorPausedDescription:
      'Call or WhatsApp Habesha Real Estate Advisory for indicative Ayat and Temer pricing guidance in Addis Ababa. Free advisory — no buyer fees.',
    listingDescriptionFallback:
      'View photos, specs, and pricing for this home in Addis Ababa. Free enquiry through Habesha Real Estate Advisory — no buyer fees.',
    projectDescriptionFallback:
      'Browse homes and layouts at this Addis Ababa development. Free buyer advisory from Habesha Real Estate Advisory.',
  },
  apartments: {
    heroEyebrow: 'Partner apartments',
    heroTitle: 'Residential',
    heroTitleAccent: 'location',
    heroBody: 'Choose a location to browse homes.',
    heroBodyFiltered: 'Choose a location to browse homes.',
    heroEyebrowAyat: 'Ayat Real Estate',
    heroEyebrowTemer: 'Temer Properties',
    heroTitleFiltered: 'Browse',
    heroTitleAccentAyat: 'Ayat homes',
    heroTitleAccentTemer: 'Temer homes',
    statDevelopments: 'Developments',
    statHomes: 'Homes listed',
    resultsHint: '{{developments}} developments · {{homes}} homes',
    pickLocation: 'Tap a location to see homes at that place.',
    filterAll: 'All developers',
    filterAyat: 'Ayat',
    filterTemer: 'Temer',
  },
  shops: {
    heroEyebrow: 'Ayat commercial shops',
    heroTitle: 'Shops',
    heroTitleFiltered: 'Shop locations',
    heroBody: 'Choose a location for floor rates.',
    heroBodyFiltered: 'Choose a location for floor rates.',
    pickLocation: 'Tap a location to see official floor rates per m².',
    loading: 'Loading shop locations…',
    noLocations:
      'No shop locations are published yet. Call or WhatsApp us for guidance.',
    emptyTitleFiltered: 'No {{brand}} shops published yet',
    emptyBodyFiltered:
      'We do not list shop locations for {{brand}} yet. Call or WhatsApp Habesha Real Estate Advisory for guidance — or browse residential locations instead.',
    commercial: 'Commercial shop',
    developer: 'Ayat Share Company',
    tapForDetails: 'View floor rates',
    calculatorNote: 'You can also open the full shop calculator for any zone.',
    openCalculator: 'Open shop calculator',
    backToLocations: 'All shop locations',
    notFoundTitle: 'Location not found',
    notFoundBody: 'This shop zone is not in our catalogue.',
    locationDetailIntro:
      'Official Ayat Section 11 rates per floor. Call or WhatsApp us for a quote on your shop size.',
    officialRatesTitle: 'Official price per m²',
    floorColumn: 'Floor',
    pricePerSqmColumn: 'Price',
    ratesNote: 'Prices include VAT where applicable. Final quotes are confirmed with Ayat through our advisors.',
    ratesPending:
      'Floor rates for this zone are not published yet. Call or WhatsApp us for a quote.',
    estimateTitle: 'Estimate your shop',
    developerBadge: '{{developer}} shops',
    whatsappPrefill: 'Hello, I am interested in shops at {{location}} on Habesha Real Estate Advisory.',
    contactHint: 'Call or WhatsApp for a quote on this shop location.',
  },
  home: {
    heroEyebrow: 'Modern real estate · Addis Ababa',
    heroTitle: 'Pick a',
    heroTitleAccent: 'developer',
    heroTitleEnd: 'to begin.',
    heroBody:
      'We advise you on the best homes from {{ayatBrand}} and {{temerBrand}} based on your budget, area, and lifestyle. {{advisoryNotAyat}} Compare developers, then enquire through us.',
    heroMinimalBody: 'Compare Ayat and Temer real estate — then open residential or shops.',
    advisoryNotAyat: 'We are not a developer.',
    heroFreeLine: '100% free advice — no fees and no commission from you.',
    advisoryEyebrow: 'Free advisory',
    advisoryTitle: 'The right home for your preferences — completely free',
    advisorySummary:
      'Tell us what you are looking for. We compare trusted developers and guide you to the best match. Our service is free for buyers: no advisory fees and no commission charged to you.',
    advisoryPoint1Title: 'Matched to your preferences',
    advisoryPoint1Text:
      'Bedrooms, neighbourhood, budget, and payment plan — we shortlist options that fit what you want, not a one-size-fits-all pitch.',
    advisoryPoint2Title: 'No fees from you',
    advisoryPoint2Text:
      'You never pay Habesha Real Estate Advisory for guidance, comparisons, or enquiries.',
    advisoryPoint3Title: 'No buyer commission',
    advisoryPoint3Text:
      'We do not take any commission from buyers. Everything we offer is free.',
    advisoryCta: 'Get free advice',
    advisoryCtaHint: 'Call or WhatsApp — always free for you.',
    freeBadge: '100% free',
    exploreListings: 'Explore locations',
    pickDeveloperTitle: 'Choose a partner developer',
    pickDeveloperHint: 'Residential or shops next',
    ctaAyat: 'See Ayat homes & shops',
    ctaTemer: 'See Temer homes & shops',
    marqueeTagline: 'Trusted partner developers on Habesha Real Estate Advisory',
    ayatHomes: 'Ayat homes',
    priceCalculator: 'Estimate Ayat price',
    statDevelopers: 'Partner developers',
    statExperience: 'Years of Ayat experience',
    statYou: 'Choose without limits',
    inventoryEyebrow: 'Partner developers',
    inventoryTitle: 'Start with Ayat or Temer',
    partnersEyebrow: 'Our partners',
    partnersTitle: 'Developers you can compare with us',
    partnersDescription:
      'Each partner below is a full showcase — browse residential homes or commercial shops. New partners appear here when published in the CMS.',
    browseDevelopers: 'Browse developers',
    developersCta: 'View {{name}}',
    developersOfficialSite: 'Official website',
    developersCardFallbackDescription:
      'Explore residential and commercial options from {{name}} with free Habesha advice.',
    developersEmptyTitle: 'Developers coming soon',
    developersEmptyBody:
      'Partner developers are not published yet. Call or WhatsApp us and we will guide you personally.',
    developersErrorTitle: 'Could not load developers',
    developersErrorBody: 'Please try again shortly, or contact us for free advice.',
    developersContactPrefill: 'Hello — I would like help choosing a developer on Habesha Homes.',
    inventoryDescription:
      'Pick a developer, then choose Residential or Shops to see that company’s locations.',
    cardAyatTitle: 'Ayat apartments & shops',
    cardAyatDescription:
      'Ayat Hills, CMC, and other Ayat developments. Commercial shops use Ayat Section 11 locations.',
    cardAyatShopsHint: 'Ayat commercial shops:',
    cardTemerTitle: 'Temer apartments',
    cardTemerDescription:
      'Sarbet City Plus, Blue Point, Aware, Gelan Shopping Center, and more Temer listings.',
    cardCommercialTitle: 'Commercial shops',
    cardCommercialDescription:
      'Estimate shop prices by Ledeta, Kazanchis, Bole Air, and other Section 11 locations — separate from apartments.',
    cardTagHomes: 'Homes',
    cardTagShops: 'Shops',
    viewListings: 'View listings',
    viewShopEstimate: 'Estimate shop price',
    credibilityEyebrow: 'Partner credibility',
    credibilityTitle: 'Trusted partner developers',
    credibilityDescription:
      'Ayat and Temer each publish their own projects. We help you compare both on one site.',
    ctaEyebrow: 'Get in touch',
    ctaTitle: 'Ready to find your home?',
    ctaDescription: 'Browse available units, see price previews, and message us when you are ready.',
    browseListings: 'Browse listings',
    staffLogin: 'Staff login',
  },
  developer: {
    heroEyebrow: 'Partner developer',
    heroBody: 'What would you like to browse from {{brand}}?',
    chooseKindHint: 'Residential homes or commercial shops',
    kindSwitcherLabel: 'Product type',
    kindResidential: 'Residential',
    kindResidentialEyebrow: 'Apartments & homes',
    kindResidentialBody: 'Browse this developer’s residential locations and layouts.',
    kindShops: 'Shops',
    kindShopsEyebrow: 'Commercial',
    kindShopsBody: 'Browse this developer’s shop locations and floor rates.',
    browseResidential: 'See residential locations',
    browseShops: 'See shop locations',
    backHome: '← Back to home',
    backToKind: '← Residential or shops',
    contactHint: 'Call or WhatsApp anytime — advisory is free for buyers.',
    whatsappPrefill: 'Hello, I am interested in {{brand}} on Habesha Real Estate Advisory.',
  },
  temer: {
    featuredEyebrow: 'Featured partner developer',
    body: 'Temer Properties builds modern apartments in Sarbet, Aware, Ayat, Piyassa, and across Addis Ababa. Here you browse their available units and enquire; purchase stays with Temer.',
    browseTemer: 'Browse Temer homes here',
    officialSite: 'Official Temer site ↗',
    highlights: [
      'Sarbet City Plus & Blue Point',
      'Aware & Ayat area projects',
      'Addis Ababa · contact us to enquire',
    ],
    priceOnRequestTitle: 'Price on request',
    priceOnRequestBody:
      'Temer pricing is confirmed with their sales team. Call {{phone}} and we will help you reach Temer.',
    temerCalculatorLink: 'Temer’s online estimator ↗',
  },
  temerCard: {
    beds: 'Beds',
    baths: 'Baths',
    size: 'Size',
    specDash: '—',
    viewDetails: 'View details',
    browseIntro:
      'Discover Temer apartments for sale in Addis Ababa — residential and commercial listings through Habesha Real Estate Advisory.',
  },
  ayat: {
    years: 'Years',
    featuredEyebrow: 'Featured partner developer',
    reputation: 'That reputation belongs to Ayat.',
    body: "Ethiopia's established developer: Ayat Hills, CMC, Grand Mall, and communities built over decades. {{reputation}} Here you only browse and compare. Purchase and contracts stay with the developer you choose.",
    browseAyat: 'Browse Ayat homes here',
    officialSite: 'Official Ayat site ↗',
    highlights: [
      'Ayat Hills, CMC, and major Addis Ababa projects',
      'Residential and commercial properties',
      'One of Ethiopia’s most recognized developers',
    ],
    stats: [
      { value: '25+', label: 'years of experience (Ayat)' },
      { value: '15k+', label: 'jobs created (Ayat)' },
      { value: '5k+', label: 'happy customers (Ayat)' },
    ],
  },
  belayRole: {
    eyebrow: 'How we work',
    title: 'What Habesha Real Estate Advisory is',
    summary:
      'Habesha Real Estate Advisory is not a developer. We advise buyers on the best real estate for their preferences, compare homes from multiple companies, and connect you with the right developer — all at no cost to you.',
    feature1Title: 'Developer on every listing',
    feature1Text:
      'Every listing shows the developer name. You always know who built the project and who you buy from.',
    feature2Title: 'Compare before you enquire',
    feature2Text: 'Compare areas, unit types, and indicative prices in one place before you enquire.',
    feature3Title: 'Enquire without confusion',
    feature3Text:
      'Send one enquiry through us and we help you reach the right developer. No pressure to pick one brand.',
  },
  listings: {
    heroEyebrow: 'Browse · Compare · Enquire',
    heroTitle: 'Homes from Ayat, Temer, and more',
    heroBody:
      'Each listing names the builder — {{ayatBrand}} or {{temerBrand}}. Habesha Real Estate Advisory helps you compare. We do not replace the developer on your purchase.',
    loadError: 'Could not load listings. Check that the API is running, then refresh.',
    emptyTitle: 'No matches yet',
    emptyDescription: 'Try another area or bedroom count, or browse all partner homes.',
    showAll: 'Show all',
    ayatHomesOnly: 'Ayat homes only',
    shopsCalculatorNote:
      'Commercial shops use different Ayat locations than apartments. Browse homes below or estimate a shop price.',
    estimateShop: 'Shop price estimate',
    quickAyatArea: 'Ayat area',
    quickLideta: 'Lideta',
    quickKazanchis: 'Kazanchis',
    quickBole: 'Bole (Belair)',
    quickCmc: 'CMC neighborhood',
    quick2br: '2 bedrooms',
    quick3br: '3 bedrooms',
    quickAyatCo: 'Ayat Share Company',
    browseByLocation:
      'Browse by location first. Tap a place to see every bedroom layout and finish available there.',
  },
  projectBrowse: {
    developerApartments: '{{developer}} apartments',
    homesHere: 'Homes at this location',
    tapForLayouts: 'View location details',
    locationIntroFallback: 'Browse layouts, media, and floor rates for this development below.',
    ayatLocationIntro:
      'Explore home types at this Ayat development, watch progress photos and video, and review floor rates — then call or WhatsApp us.',
    layoutsAvailable: 'layouts available',
    layoutAvailable: 'layout available',
    viewLayouts: 'View layouts',
    layoutsTitle: 'Available layouts',
    layoutsSubtitle: '{{count}} home types at this location — photos and details from our listings.',
    mediaTitle: 'Photos & video',
    mediaSubtitle: 'Drone footage and gallery images for this development.',
    videoNav: 'Video',
    videoBadge: 'Site tour',
    galleryTitle: 'Photos',
    gallerySubtitle: 'More images from this development.',
    calculatorHeadline: 'Calculate an indicative Ayat price',
    calculatorHint: 'Estimate price for this location using official Ayat rates.',
    oneHome: '1 example home',
    homeCount: '{{count}} example homes',
    bedroomCount: '{{count}} bedrooms',
    chooseLayout:
      'Each layout below is a real home type at this development. Compare bedrooms, finish, and size — then call or WhatsApp us for guidance.',
    layoutsIntro:
      'Homes are grouped by building type. Specs and photos are on this page; floor rates and contact are below.',
    filterBedrooms: 'Bedrooms',
    priceEstimate: 'Price estimate',
    officialRatesTitle: 'Official price per m² by floor',
    floorColumn: 'Floor band',
    homeTypeColumn: 'Home type',
    pricePerSqmColumn: 'Price',
    ratesNote:
      'Indicative ETB/m² including VAT where applicable. Final quotes are confirmed through Habesha Real Estate Advisory.',
    ratesNav: 'Floor rates',
    contactNav: 'Contact',
    whatsappPrefill: 'Hello, I am interested in {{location}} on Habesha Real Estate Advisory.',
    contactHint: 'Call or WhatsApp about this location — advisory is free.',
    priceOnRequestTitle: 'Price on request',
    priceOnRequestBody:
      'Floor rates for this location are not published yet. Call {{phone}} or WhatsApp us for an indicative quote.',
    backToLocations: 'All locations',
    loading: 'Loading homes…',
    missingProject: 'Missing location.',
    notFoundTitle: 'Location not found',
    notFoundBody: 'This development may no longer be listed. Browse all locations instead.',
    noBedroomMatch: 'No homes match this bedroom count at this location.',
    showAllLayouts: 'Show all layouts',
  },
  buildingType: {
    mixedTitle: 'Mixed use',
    duplexTitle: 'Duplex',
    flatTitle: 'Flat apartments',
    mixedIntro:
      'Shops and mall on ground to floor {{retailMax}}. Apartments from floor {{residentialMin}} upward.',
    mixedIntroFallback: 'Combined retail and residential towers.',
    duplexIntro: 'Duplex apartments at this location.',
    flatIntro: 'Residential-only towers — no shop floors.',
    shopFloorsHint: 'For shop floors in mixed towers,',
    shopCalculatorLink: 'use the shop price calculator',
    shopRatesLink: 'view shop floor rates',
    otherLayouts: 'Other layouts',
    otherLayoutsHint: 'Homes not yet tagged with a building type.',
  },
  layoutCard: {
    bedrooms: 'Bedrooms',
    finish: 'Finish',
    size: 'Size',
    floor: 'Floor',
    notSpecified: '—',
    viewDetails: 'View full details',
    viewDetailsHint: 'Photos, specs & indicative price',
    buildingMixedApt: 'Mixed · Apartment',
    buildingMixedShop: 'Mixed · Shop floor',
    sectionEyebrowMixed: 'Building type',
    sectionEyebrowDuplex: 'Building type',
    sectionEyebrowFlat: 'Building type',
    sectionEyebrowAll: 'Available homes',
    sectionEyebrowOther: 'More layouts',
  },
  filters: {
    findHome: 'Find your home',
    loading: 'Loading…',
    property: 'property',
    properties: 'properties',
    available: 'available',
    locationsAndHomes: '{{locations}} locations · {{homes}} homes',
    clearAll: 'Clear all',
    any: 'Any',
    optionsLoading: 'Loading filter options…',
    neighborhood: 'Neighborhood / location',
    neighborhoodPlaceholder: 'Type or select a location…',
    bedrooms: 'Bedrooms',
    bedroomsPlaceholder: 'Type or select bedrooms…',
    search: 'Search homes',
    city: 'City',
    cityPlaceholder: 'Type or select a city…',
    developer: 'Developer',
    developerPlaceholder: 'Type or select a developer…',
    unitType: 'Unit finish and size',
    unitTypePlaceholder: 'Type or select unit type…',
  },
  listingCard: {
    forSale: 'For sale',
    photoSoon: 'Photo coming soon',
    galleryPhotos: '{{count}} photos',
    galleryPrev: 'Previous photo',
    galleryNext: 'Next photo',
    galleryGoTo: 'Show photo {{n}}',
    galleryPosition: '{{current}} / {{total}}',
    defaultCity: 'Addis Ababa',
    locatedIn: '{{place}}',
    titleTemplate: '{{bedrooms}}, {{finish}}, {{project}}',
    semiFinished: 'semi-finished',
    regularFinished: 'regular finished',
    bed: 'bed',
    beds: 'beds',
  },
  listingDetail: {
    missingSlug: 'Missing listing slug.',
    loading: 'Loading…',
    notFound: 'Listing not found or no longer available.',
    backToListings: 'Back to listings',
    backToLocation: 'Back to this location',
    backToApartments: 'All apartment locations',
    allListings: '← All listings',
    locationPending: 'Location to be confirmed',
    bedrooms: 'bedrooms',
    unit: 'Unit',
    floor: 'Floor number',
    noGallery: 'No gallery images yet.',
    aboutHome: 'About this home',
    priceLoading: 'Loading indicative price…',
    pricingEyebrow: 'Pricing',
    priceOnRequest: 'Price on request',
    priceOnRequestBody:
      'Published pricing is not available for this listing yet. Contact us for a formal quote.',
    indicativePrice: 'Indicative price',
    inclVat: 'including value added tax',
    paymentEyebrow: 'Payment plan preview',
    plan: 'Plan',
    planUnavailable: 'This plan is not available for this unit.',
    calculating: 'Calculating schedule…',
    whatsapp: 'WhatsApp about this home',
    whatsappHint: 'Set {{var}} in {{env}} to enable WhatsApp (E.164, no +).',
    enquiryTitle: 'Request information',
    name: 'Name',
    phone: 'Phone',
    emailOptional: 'Email (optional)',
    message: 'Message',
    messagePlaceholder: 'When would you like to visit?',
    submitError: 'Could not submit. Check your details and try again.',
    sending: 'Sending…',
    sendEnquiry: 'Send enquiry',
    thankYou: 'Thank you. We received your enquiry for {{slug}}. Our team will contact you soon.',
    calculatorEyebrow: 'Ayat pricing',
    tabOverview: 'Overview',
    tabDetails: 'Details',
    tabFeatures: 'Features',
    tabMap: 'Map',
    interestedIn: "I'm interested in {{title}}",
    specCategory: 'Category',
    specBedrooms: 'Bedrooms',
    specBathrooms: 'Bathrooms',
    specSize: 'Property size',
    specDelivery: 'Delivery',
    specLocation: 'Location',
    featuresInterior: 'Interior details',
    featuresOutdoor: 'Outdoor details',
    featuresUtilities: 'Utilities',
    featuresOther: 'Other features',
    commercial: 'Commercial',
    apartment: 'Apartment',
    noDetails: 'No additional details published for this listing yet.',
    noFeatures: 'No feature list published for this listing yet.',
    noMap: 'Map location is not available for this listing yet.',
    salesOffice: 'Sales office',
    scheduleShowing: 'Schedule a showing?',
    morePhotos: 'more photos',
    call: 'Call',
    temerSales: 'Temer sales',
  },
  calculator: {
    pausedEyebrow: 'Pricing guidance',
    pausedTitle: 'Contact us for pricing',
    pausedBody:
      'The self-service price calculator is paused for now. Floor rates still appear on location pages where published. Call or WhatsApp our free advisory team for an indicative quote.',
    pausedBrowseHomes: 'Browse apartments',
    pausedBrowseShops: 'Browse shops',
    eyebrow: 'Ayat pricing · Self-service estimate',
    title: 'Calculate an indicative Ayat price',
    intro:
      'Ayat uses two different location lists: apartments (Section 10 of their sales strategy) and commercial shops (Section 11). Start by choosing apartment or shop, then pick the location, size, and floor. We do not sell property. This tool helps you compare before you contact Ayat or our team.',
    propertyKindTabsLabel: 'Property type',
    apartmentTab: 'Apartments',
    shopTab: 'Commercial shops',
    propertyKindPickHint: 'Choose apartments or commercial shops above to see the correct locations.',
    apartmentLocationsHint:
      'Apartment areas from Ayat strategy Section 10 (Lideta, Kazanchis, Bole, CMC, Ayat Hills). Price per square meter depends on floor range and construction stage.',
    shopLocationsHint:
      'Shop areas from Ayat strategy Section 11 (Ledeta, Kazanchis, Bole Air, Zone 2, Meri Luke 1, Summit, and more). Different from apartment locations.',
    pricingLoadFailed:
      'Live apartment rates could not be loaded from the server. Open Admin → Pricing and confirm CMC rows are saved, then restart the API. Until that works, the calculator cannot show a price.',
    loadingRates: 'Loading apartment rates from admin Pricing…',
    noRatesLoaded:
      'Live apartment rates did not load. Open Admin → Pricing, confirm CMC rows are saved with Construction stage “Near completion”, then restart the API and refresh this page.',
    noFloorsForRates: 'No floors priced for this selection',
    liveRatesNote: 'Rates reflect current admin pricing (updated when you save in Pricing).',
    disclaimer:
      'Indicative only. Figures follow Ayat’s published sales strategy structure and our current price table estimates. Official quotes from Ayat Share Company are required before purchase. Prices may change quarterly.',
    step1Title: 'What are you looking for?',
    kindHome: 'Apartment (home)',
    kindHomeHint: 'One, two, or three bedrooms · semi-finished or regular finished',
    kindShop: 'Commercial shop',
    kindShopHint: 'Price per square meter by shop floor · milestone payments only',
    step2AreaTitle: 'Which area?',
    step2ShopZoneTitle: 'Shop location',
    zones: {
      lideta: 'Lideta, Addis Ababa',
      kazanchis: 'Kazanchis, Addis Ababa',
      bole: 'Bole, Addis Ababa',
      cmc: 'CMC neighborhood, Addis Ababa',
      ayatMainVillage: 'Ayat (Main Village), Addis Ababa',
    },
    projects: {
      lideta: 'Lideta — building not started',
      kazanchis: 'Kazanchis — construction started',
      bole: 'Bole — building not started',
      cmc: 'CMC residential towers',
      cmcUnstarted: 'CMC and others — building not started',
      cmcNearCompletion: 'CMC and others — near completion',
      ayatHills: 'Ayat Hills (Main Village)',
    },
    completionLabel: 'Construction stage at CMC',
    completionUnstarted: 'Building not yet started or early construction',
    completionNear: 'Building near completion',
    completionHint:
      'Ayat uses different price per square meter tables for buildings that are unstarted versus near completion. Near completion uses a higher rate at CMC in this estimate.',
    step3BedTitle: 'Bedrooms and finish',
    step3BedHint:
      'Choose how many bedrooms you need, then whether you want semi-finished or regular finished.',
    oneBedroom: '1 bedroom',
    twoBedrooms: '2 bedrooms',
    threeBedrooms: '3 bedrooms',
    bedrooms: '{{count}} bedrooms',
    finishLabel: 'Finish level',
    finishSemi: 'Semi-finished apartment',
    finishSemiHint: 'Shell with basic fittings. Lower base price, faster delivery target.',
    finishRegular: 'Regular finished apartment',
    finishRegularHint: 'Full interior finish. Higher price per square meter on supported types.',
    finishLockedHint: 'Indicative calculator uses semi-finished rates only for now.',
    step3ShopFloorTitle: 'Which floor is the shop on?',
    step3ShopFloorHint: 'Ground floor shops usually cost more per square meter than upper floors.',
    shopFloor: {
      GF: 'Ground floor',
      '1F': 'First floor',
      '2F': 'Second floor',
      '3F': 'Third floor',
    },
    shopZones: {
      ledeta: 'Ledeta',
      kazanchis: 'Kazanchis',
      boleAir: 'Bole Air',
      zone2: 'Zone 2',
      meriLuke1: 'Meri Luke 1',
      summit: 'Summit',
      zone3University: 'Zone 3 (University area)',
      zone8Linda: 'Zone 8 (Linda)',
    },
    boleAirGroundOnly:
      'At Bole Air, Ayat’s published shop prices apply to ground floor only. Other floors are not listed in Section 11.',
    unitTypeUnknown: 'Apartment type (confirm with Ayat sales)',
    unitTypes: {
      SFCA: 'Two-bedroom semi-finished apartment, Type A',
      SFCR: 'Three-bedroom semi-finished apartment, Type R',
      RFCA: 'Two-bedroom regular finished apartment, Type A',
      RFCR: 'Three-bedroom regular finished apartment, Type R',
    },
    squareMetersValue: '{{value}} square meters',
    perSquareMeter: 'per square meter',
    floorBandRange: 'Floors {{from}} through {{to}}',
    embeddedEyebrow: 'Price calculator',
    embeddedTitle: 'Estimate for this listing',
    embeddedIntro:
      'Prefilled from this home. Change the floor, size, or down payment level to see how the list price, client discount, and payment milestones change.',
    embeddedProject: 'Project',
    embeddedBedrooms: 'Bedrooms',
    embeddedSize: 'Unit size',
    embeddedFloor: 'Floor level',
    embeddedAdjustTitle: 'Adjust floor, size, or construction stage',
    embeddedAdjustHint:
      'Price per square meter depends on the floor range. Try another floor or size to compare before you contact sales.',
    embeddedPaymentTitle: 'Down payment and client discount',
    embeddedPaymentHint:
      'Choose how much you pay upfront. The client discount percentage updates automatically.',
    openFullCalculator: 'Open full calculator (all apartment areas) →',
    openShopCalculator: 'Estimate a commercial shop →',
    step4SizeTitle: 'Unit size (net square meters)',
    step4SizeHint:
      'Pick the net area that matches your unit type. Ayat allows about five percent variation on signed contracts.',
    customSize: 'Or enter size in square meters',
    shopSizeRange: 'Shops: {{min}} to {{max}} square meters depending on category',
    step5FloorTitle: 'Apartment floor',
    step5FloorHint:
      'Price per square meter is set by floor range (lower floors usually cost more). Select the floor you want.',
    floorN: 'Floor {{n}}',
    step6PaymentTitle: 'Down payment and payment plan',
    step6PaymentHint:
      'Client discount depends on how much you pay upfront. The sixty-forty plan uses a five percent discount with forty percent down and sixty percent on handover.',
    tiers: {
      t100: 'Pay one hundred percent upfront',
      t85: 'Pay eighty-five percent upfront',
      t70: 'Pay seventy percent upfront',
      t55: 'Pay fifty-five percent upfront',
      t40: 'Pay forty percent upfront',
      t35: 'Pay thirty-five percent upfront',
      t6040:
        'Sixty-forty plan (forty percent down payment, sixty percent when you receive the keys)',
    },
    tierDiscount: '{{percent}} percent client discount on list price',
    resultTitle: 'Your estimate',
    resultPending:
      'Complete the steps on the left to see price per square meter, discount, and payments.',
    compactAdjustHint: 'Choose your options — your estimate updates below.',
    compactDisclaimer:
      'Indicative only — official Ayat quotes are required before purchase.',
    compactFieldKind: 'Property type',
    compactFieldArea: 'Apartment area',
    compactFieldShopZone: 'Shop location',
    compactFieldBedrooms: 'Bedrooms',
    compactFieldFinish: 'Finish level',
    compactFieldSize: 'Unit size (m²)',
    compactFieldFloor: 'Apartment floor',
    compactFieldShopFloor: 'Shop floor',
    compactFieldPayment: 'Down payment plan',
    compactFieldCompletion: 'Construction stage',
    pricePerSqm: 'Price per square meter (including value added tax, indicative)',
    floorBand: 'Floor range: {{band}}',
    shopFloorSelected: 'Shop floor: {{floor}}',
    unitType: 'Unit type: {{label}}',
    listPrice: 'List price (before discount)',
    listPriceHint: 'Unit size multiplied by price per square meter for your floor range',
    clientDiscount: 'Client discount',
    clientDiscountHint:
      'From Ayat’s discount schedule based on your selected down payment level. Group purchase bonuses are not included.',
    locationPromotion: 'Special offer: {{name}}',
    locationPromotionHint:
      'Limited-time extra discount for this location, applied after the Ayat tier discount.',
    priceAfterDiscount: 'Total price (all discounts)',
    totalAfterTier: 'Total after down-payment discount',
    pricePerSqmAfterTier: 'Per m² after down-payment discount',
    totalAfterPromotion: 'Total after special offer',
    pricePerSqmAfterPromotion: 'Per m² after special offer',
    finalSummaryTitle: 'Final summary (all discounts)',
    effectivePricePerSqmFinal: 'Per m² — all discounts included',
    effectivePricePerSqmSummary:
      '{{area}} — list {{listRate}}/m² → after down-payment {{tierRate}}/m² → final {{finalRate}}/m².',
    downPayment6040: 'Down payment now (sixty-forty plan)',
    balance6040: 'Balance due on handover',
    firstMilestone: 'Due when you sign the contract (first milestone)',
    downPaymentNow: 'Down payment due now',
    remainingBalance: 'Remaining balance after this payment',
    milestoneTitle: 'Milestone payment schedule',
    milestoneHint:
      'Percentages from Ayat payment schedule. Amounts apply to the price after discount.',
    shopNoInstallments:
      'Long-term installment plans are not available for commercial shops. Only milestone down payments apply.',
    oneBedNote:
      'One-bedroom pricing uses the same semi-finished two-bedroom Type A price band as a guide. Confirm the exact unit type with Ayat sales.',
    noPriceForSelection:
      'No ETB/m² rate in admin Pricing for this bedroom count, finish, floor, and construction stage. Add a price row whose floor band includes this floor (e.g. 11-18 for floor 16) and the right unit code (SFCA for 1–2 bed semi-finished, SFCR for 3 bed semi-finished).',
    howItWorksTitle: 'How this calculator works',
    how1:
      'We multiply your net area in square meters by the rate for your floor range and unit type (semi-finished or regular finished, two or three bedrooms).',
    how2: 'We apply the client discount percentage for the down payment level you selected.',
    how3:
      'We show either the cash due now (partial upfront payment) or Ayat’s milestone schedule (full payment or sixty-forty plan).',
    how4: 'Contact Ayat Share Company or us at +251962750710 for a formal quote before signing.',
    milestones: {
      signing: 'On signing contract',
      month4: 'Within 4 months',
      month8: 'Within 8 months',
      month12: 'Within 12 months',
      month18: 'Within 18 months',
      month24: 'Within 24 months',
      structure: 'Structure / masonry complete',
      handover: 'On handover & keys',
    },
  },
}

export type Messages = typeof en
