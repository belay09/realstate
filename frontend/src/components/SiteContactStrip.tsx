...
export function SiteContactBanner({
  className = '',
  whatsAppMessage,
  hint,
}: {
  className?: string
  /** Prefill WhatsApp with location / listing context. */
  ...
  return (
    <div
      className={`flex flex-col gap-5 rounded-2xl border border-border bg-gradient-to-br from-slate-950 via-slate-900 to-brand-950 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:gap-6 sm:px-7 sm:py-6 ${className}`}
    >
      <div className="flex items-start gap-3 sm:items-center">
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-brand-500 text-white shadow-md shadow-brand-900/30 sm:h-12 sm:w-12">
        ...
        <div className="min-w-0">
          ...
          <a
            href={SITE_CONTACT.telHref}
            className="mt-1 block text-xl font-bold tracking-tight text-white transition hover:text-brand-200 sm:text-2xl"
          >
            {SITE_CONTACT.phoneDisplay}
          </a>
          <p className="mt-1.5 text-sm leading-relaxed text-slate-400">{hint ?? t('contact.bannerHint')}</p>
        </div>
      </div>
      <div className="flex w-full flex-col gap-2 sm:w-auto sm:shrink-0 sm:flex-row sm:flex-wrap">
        <a
          href={SITE_CONTACT.telHref}
          className="btn-luxury-light w-full justify-between py-2 pl-5 pr-1.5 text-xs normal-case tracking-normal sm:w-auto"
        >
          <span>{t('contact.call')}</span>
          <span className="btn-luxury-icon h-8 w-8 bg-slate-950 text-white">
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex w-full items-center justify-center rounded-full border border-white/30 px-5 py-2.5 text-xs font-semibold text-white transition hover:bg-white/10 sm:w-auto"
        >
          {t('contact.whatsapp')}
        </a>
      </div>
